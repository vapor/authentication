                      ...
                      size_t encryptedlen)
{
    ...
    VaporAuth_Blowfish_initstate(&state);
    VaporAuth_Blowfish_expandstate(&state, csalt, salt_len,
                               (u_int8_t *) key, key_len);
    for (k = 0; k < rounds; k++) {
        VaporAuth_Blowfish_expand0state(&state, (u_int8_t *) key, key_len);
        VaporAuth_Blowfish_expand0state(&state, csalt, salt_len);
    }

    /* This can be precomputed later */
    j = 0;
    for (i = 0; i < BCRYPT_WORDS; i++)
        cdata[i] = VaporAuth_Blowfish_stream2word(ciphertext, 4 * BCRYPT_WORDS, &j);

    /* Now do the encryption */
    for (k = 0; k < 64; k++)
        vapor_auth_blf_enc(&state, cdata, BCRYPT_WORDS / 2);
    for (i = 0; i < BCRYPT_WORDS; i++) {
